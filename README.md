...
LLMベースのAIエージェントは自律的にツールを呼び出して業務を遂行しますが、「誰が・何を・いつ実行したか」を人間が把握・制御する仕組みが不足しています。ApprovalGate はこの問題に対し、エージェントの実行ループにゲートを設けることで、組織としてAI活動を監査・承認できる基盤を提供します。

### 主な機能

| 機能 | 説明 |
|------|------|
| 承認リクエスト管理 | エージェントからのリクエストを受け付け、承認・拒否・タイムアウトのステータスで管理 |
| ルールエンジン | プロジェクト・操作種別・パラメータ値などの条件でリクエストを自動承認／自動拒否 |
| Slack連携 | 承認リクエストをSlackに通知し、ボタン操作で承認・拒否が可能 |
| マルチプロジェクト | テナント内に複数プロジェクトを作成し、プロジェクトごとにAPIキーとルールを管理 |
| 監査ログ | すべての承認アクションをログとして記録し、後から追跡可能 |
| アナリティクス | 自動承認率・平均応答時間・プロジェクト別リクエスト数などの統計を可視化 |
| タイムアウト処理 | 一定時間内に承認されなかったリクエストを自動でタイムアウト処理（fail-open / fail-closed 設定可） |

### 利用イメージ

```
AIエージェント
  │ SDK経由でリクエスト送信（例: 「顧客データをCSVエクスポートしてよいか？」）
  ▼
ApprovalGate（ゲート）
  ├─ ルール合致 → 即時自動承認・拒否
  └─ 人間レビュー → WebダッシュボードまたはSlackで承認
  │
  ▼ 承認結果をSDKに返却
AIエージェント → 処理を続行 or 中断
```
